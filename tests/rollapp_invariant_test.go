...
func TestRollappInvariant_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	maxIdleTime1 := "3s"
	maxProofTime := "500ms"
	configFileOverrides1 := overridesDymintToml(settlement_layer_rollapp1, settlement_node_address, rollapp1_id, gas_price_rollapp1, maxIdleTime1, maxProofTime, "60s")
	...
	maxIdleTime2 := "3s"
	configFileOverrides2 := overridesDymintToml(settlement_layer_rollapp2, settlement_node_address, rollapp2_id, gas_price_rollapp2, maxIdleTime2, maxProofTime, "60s")
	...
	// wait one more time as test spec requires that
	isFinalized, err = dymension.WaitUntilRollappHeightIsFinalized(ctx, rollapp1.GetChainID(), rollappHeight, 300)
	require.NoError(t, err)
	require.True(t, isFinalized)

	// loop for the pending states
	for {
	...
	_, err = dymension.GetNode().CrisisInvariant(ctx, dymensionUser.KeyName(), "rollapp", "rollapp-by-eip155-key")
	require.NoError(t, err)

	_, err = dymension.GetNode().CrisisInvariant(ctx, dymensionUser.KeyName(), "rollapp", "rollapp-finalized-state")
	require.NoError(t, err)
...
func TestRollappInvariant_Wasm(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
