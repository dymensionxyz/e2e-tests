...
func TestRollAppFreeze_EVM(t *testing.T) {
	...
	SetupRelayer(ctx, t, eRep, dymension, r1, dymension.Config().ChainID, rollapp1.Config().ChainID, ibcPath)

	err = testutil.WaitForBlocks(ctx, 5, dymension, rollapp1)
	require.NoError(t, err)

  SetupRelayer(ctx, t, eRep, dymension, r2, dymension.Config().ChainID, rollapp2.Config().ChainID, anotherIbcPath)
...
func TestOtherRollappNotAffected_EVM(t *testing.T) {
	...
	SetupRelayer(ctx, t, eRep, dymension, r, dymension.Config().ChainID, rollapp1.Config().ChainID, ibcPath)

	err = testutil.WaitForBlocks(ctx, 10, dymension, rollapp1, rollapp2)
	require.NoError(t, err)

	SetupRelayer(ctx, t, eRep, dymension, s, dymension.Config().ChainID, rollapp2.Config().ChainID, anotherIbcPath)
