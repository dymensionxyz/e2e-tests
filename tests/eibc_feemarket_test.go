...
func TestEIBC_Fee_Market_Success_EVM(t *testing.T) {
	...
	txhash, err := dymension.FullfillDemandOrder(ctx, eibcEvents[0].OrderId, marketMakerAddr, math.NewInt(100_000))
	require.NoError(t, err)
	fmt.Println(txhash)
...
func TestEIBCUpdateOnAckErrAndTimeout_Wasm(t *testing.T) {
	...
	require.NoError(t, err)
	fmt.Println("Balance of marketMakerAddr after fulfilling the order:", balance)
	expBalanceMarketMaker := walletAmount.Add(transferAmount).Add(globalEIbcFee.MulRaw(2))
