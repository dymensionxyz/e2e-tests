...
func TestSync_BlockSync_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
...
func TestSync_BlockSync_fn_disconnect_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
...
// TestSync_Blocksync_DA_EVM In this test a full-node must sync using blocksync protocol, when it joins the network, but both nodes are also connected to DA and there are submitted batches. Full-node should sync from any of the both sources (P2P or DA). DA can be grpc or celestia.
func TestSync_Blocksync_DA_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	dymintTomlOverrides["gossiped_blocks_cache_size"] = "1"
	dymintTomlOverrides["p2p_blocksync_enabled"] = "true"
	dymintTomlOverrides["p2p_blocksync_block_request_interval"] = "10"
	...
	// block time is 200ms and batch submit time is 5s, we'll wait for 2 batch submit time => 25 blocks
	// wait for sequencer to creat x blocks
	err = testutil.WaitForBlocks(ctx, 25, rollapp1)
	require.NoError(t, err)
