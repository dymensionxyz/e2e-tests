...
func TestERC20StakingAndIBC_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	modifyEVMGenesisKV := append(
		...
		cosmos.GenesisKV{
			Key:   "app_state.mint.params.mint_epoch_identifier",
			Value: "minute",
		},
		cosmos.GenesisKV{
			...
			Value: []interface{}{
				...
				map[string]interface{}{
					...
					"description": "Denom metadata for stake",
					"display":     "stake",
					"name":        "stake",
					"symbol":      "stake",
				},
	...
	cmd := []string{"q", "staking", "validators", "--node", fmt.Sprintf("tcp://%s:26657", rollapp1.Validators[0].HostName()), "--output", "json"}

	out, _, err := rollapp1.Validators[0].ExecBin(ctx, cmd...)
	require.NoError(t, err)
	...
	rollappAdrrHex := common.BytesToAddress(rollappUser.Address())
	_, err = rollapp1.GetNode().ConvertCoin(ctx, rollappUser.KeyName(), fmt.Sprintf("%v%s", walletAmount, rollapp1.Config().Denom), rollappAdrrHex.String())
	require.NoError(t, err)

	hash, err := rollapp1.Validators[0].Delegate(ctx, rollappUser.KeyName(), response.Validators[0].OperatorAddress, "1000000000urax")
	require.NoError(t, err)
	fmt.Println(hash)

	delegation, err := rollapp1.GetNode().GetDelegation(ctx, rollappUserAddr, response.Validators[0].OperatorAddress)
	require.NoError(t, err)
	require.Equal(t, "1000000000", delegation.Balance.Amount)

	hash, err = rollapp1.GetNode().Undelegate(ctx, rollappUser.KeyName(), response.Validators[0].OperatorAddress, "100000urax")
	require.NoError(t, err)
	fmt.Println(hash)
	...
	delegation, err = rollapp1.GetNode().GetDelegation(ctx, rollappUserAddr, response.Validators[0].OperatorAddress)
	require.NoError(t, err)
	require.Equal(t, "999900000", delegation.Balance.Amount)

	command := []string{"devd", "q", "balance", rollappUserAddr, "--erc20", "--rpc", "http://ra-rollappevm_1234-1-val-0-TestERC20StakingAndIBC_EVM:8545", "--rest", "http://ra-rollappevm_1234-1-val-0-TestERC20StakingAndIBC_EVM:1317"}
	...
	require.Equal(t, walletAmount.SubRaw(999900000), balance)

	tokenPair, err := rollapp1.GetNode().QueryErc20TokenPair(ctx, "urax")
	require.NoError(t, err)
	require.NotNil(t, tokenPair)
	...
	stdout, _, err = rollapp1.Validators[0].Exec(ctx, command, nil)
	require.NoError(t, err)
	raOrigBal, err := parseRawERC20Balance(stdout)
	if err != nil {
		panic(err)
	}
	// Compose an IBC transfer and send from Hub -> rollapp
	...
	require.Equal(t, raOrigBal.Add((transferAmount.Sub(bridgingFee))), balance)
	...
	valAddress, err := rollapp1.GetNode().KeyBech32(ctx, "validator", "")
	require.NoError(t, err)

	command = []string{"devd", "q", "balance", valAddress, "--erc20", "--rpc", "http://ra-rollappevm_1234-1-val-0-TestERC20StakingAndIBC_EVM:8545", "--rest", "http://ra-rollappevm_1234-1-val-0-TestERC20StakingAndIBC_EVM:1317"}

	stdout, _, err = rollapp1.Validators[0].Exec(ctx, command, nil)
	require.NoError(t, err)

	updateBal, err := parseRawERC20Balance(stdout)
	if err != nil {
		panic(err)
	}
	require.True(t, updateBal.GT(math.ZeroInt()))
...
func TestERC20Vesting_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	err = rollapp1.GetNode().CreateKey(ctx, "test")
	require.NoError(t, err)

	address, err := rollapp1.GetNode().KeyBech32(ctx, "test", "")
	require.NoError(t, err)

	command := []string{"devd", "convert", "address", address}

	stdout, _, err := rollapp1.Validators[0].Exec(ctx, command, nil)
	require.NoError(t, err)

	hexAddress := strings.TrimSpace(string(stdout))

	_, err = rollapp1.GetNode().CreateVestingAccount(ctx, rollappUser.KeyName(), address, "100000000urax", "1893456000")
	require.NoError(t, err)

	_, err = rollapp1.GetNode().ConvertCoin(ctx, "test", fmt.Sprintf("%v%s", "100000000", rollapp1.Config().Denom), hexAddress)
	require.Error(t, err)
	...
	hash, err := rollapp1.Validators[0].Delegate(ctx, "test", response.Validators[0].OperatorAddress, "1000000urax")
	require.NoError(t, err)
	fmt.Println(hash)

	delegation, err := rollapp1.GetNode().GetDelegation(ctx, address, response.Validators[0].OperatorAddress)
	require.NoError(t, err)
	require.Equal(t, "1000000", delegation.Balance.Amount)
	...
	_, err = rollapp1.SendIBCTransfer(ctx, channel.ChannelID, address, transferData, ibc.TransferOptions{})
	require.NoError(t, err)
	...
	command = []string{"devd", "q", "balance", address, "--erc20", "--rpc", "http://ra-rollappevm_1234-1-val-0-TestERC20Vesting_EVM:8545", "--rest", "http://ra-rollappevm_1234-1-val-0-TestERC20Vesting_EVM:1317"}
...
func parseRawERC20Balance(stdout []byte) (math.Int, error) {
	scanner := bufio.NewScanner(bytes.NewReader(stdout))
	for scanner.Scan() {
		line := scanner.Text()
		if strings.HasPrefix(line, "x/erc20") {
			fields := strings.Split(line, "|")
			if len(fields) < 5 {
				return math.NewInt(0), fmt.Errorf("invalid output format")
			}
			raw := strings.TrimSpace(fields[4])
			return math.Int(math.NewUintFromString(raw)), nil
		}
	}
	return math.NewInt(0), fmt.Errorf("no erc20 raw balance found")
}
