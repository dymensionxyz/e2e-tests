...
func TestERC20StakingAndIBC_EVM(t *testing.T) {
	...
	require.NoError(t, err)
	erc20MAccAddr := erc20MAcc.Account.BaseAccount.Address
	testutil.AssertBalance(t, ctx, rollapp1, erc20MAccAddr, "urax", walletAmount.SubRaw(999900000))
	...
	testutil.AssertBalance(t, ctx, rollapp1, erc20MAccAddr, "urax", raOrigBal.Add((transferAmount.Sub(bridgingFee))))
	...
	updateErc20Bal, err := rollapp1.GetBalance(ctx, erc20MAccAddr, rollapp1.Config().Denom)
	require.NoError(t, err)
	require.True(t, updateErc20Bal.GT(erc20Bal))
