...
func TestEIBCFulfillOnOneRollApp_EVM(t *testing.T) {
	...
	res, err = dymension.GetNode().QueryPendingPacketsByAddress(ctx, rollapp2.GetChainID(), dymensionUserAddr)
	fmt.Println(res)
	require.NoError(t, err)
	...
	res, err = dymension.GetNode().QueryPendingPacketsByAddress(ctx, rollapp1.GetChainID(), marketMakerAddr)
	fmt.Println(res)
	require.NoError(t, err)

	for _, packet := range res.RollappPackets {
		txhash, err := dymension.GetNode().FinalizePacket(ctx, marketMakerAddr, rollapp1.GetChainID(), fmt.Sprint(packet.ProofHeight), fmt.Sprint(packet.Type), packet.Packet.SourceChannel, fmt.Sprint(packet.Packet.Sequence))
		require.NoError(t, err)
	...
	res, err = dymension.GetNode().QueryPendingPacketsByAddress(ctx, rollapp1.GetChainID(), rollappUserAddr)
	fmt.Println(res)
	require.NoError(t, err)
	...
	res, err = dymension.GetNode().QueryPendingPacketsByAddress(ctx, rollapp2.GetChainID(), rollapp2UserAddr)
	fmt.Println(res)
	require.NoError(t, err)
	...
	resp, err := rollapp1.GetNode().QueryPacketCommitments(ctx, "transfer", dymChannel_ra1[0].ChannelID)
	require.NoError(t, err)
	require.Equal(t, len(resp.Commitments) > 0, false, "packet commitments exist")

	resp, err = rollapp2.GetNode().QueryPacketCommitments(ctx, "transfer", dymChannel_ra2[0].ChannelID)
	require.NoError(t, err)
	require.Equal(t, len(resp.Commitments) > 0, false, "packet commitments exist")
...
func TestEIBCFulfillment_two_rollapps_EVM(t *testing.T) {
	...
	res, err = dymension.GetNode().QueryPendingPacketsByAddress(ctx, rollapp2.GetChainID(), marketMakerAddr)
	fmt.Println(res)
	require.NoError(t, err)

	for _, packet := range res.RollappPackets {
		txhash, err := dymension.GetNode().FinalizePacket(ctx, marketMakerAddr, rollapp2.GetChainID(), fmt.Sprint(packet.ProofHeight), fmt.Sprint(packet.Type), packet.Packet.SourceChannel, fmt.Sprint(packet.Packet.Sequence))
		require.NoError(t, err)
