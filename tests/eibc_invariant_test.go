...
func TestEIBCInvariant_EVM(t *testing.T) {
	...
	require.True(t, balance.Equal(transferAmount.MulRaw(2).Sub(eibcFee).Sub(eibcFee)), fmt.Sprintf("Value mismatch. Expected %s, actual %s", transferAmountWithoutFee, balance))
