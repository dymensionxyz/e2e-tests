...
func TestADMC_UserMemoTooLarge_HubtoRA_EVM(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	memoData :=  &memoData{
		MemoData: denommetadatatypes.MemoData{
			DenomMetadata: &rollappDenomMetadata,
		},
		User: &userData{"this data is over the limit of a memo length so the realyer will be stuck and revert the fund for sender, this support for the test send from hub to rollapp with memo too lange"},
	}

	// Compose an IBC transfer and send from Hub -> rollapp
	_, err = dymension.SendIBCTransfer(ctx, channel.ChannelID, dymensionUserAddr, transferData, ibc.TransferOptions{Memo: MustMarshalJSON(memoData)})
	require.NoError(t, err)
