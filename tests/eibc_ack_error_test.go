...
func TestEIBC_AckError_Dym_EVM(t *testing.T) {
	...
	err = testutil.WaitForBlocks(ctx, 15, dymension)
	require.NoError(t, err)
	...
	dymensionUser, marketMaker, rollappUser, _ := users[0], users[1], users[2], users[3]
	...
	ibcTx, err := dymension.SendIBCTransfer(ctx, channDymRollApp1.ChannelID, dymensionUserAddr, transferData, ibc.TransferOptions{})
	require.NoError(t, err)
...
func TestEIBC_AckError_3rd_Party_Token_EVM(t *testing.T) {
	...
	// Get the IBC denom for urax on Hub
	rollappTokenDenom := transfertypes.GetPrefixedDenom(channRollApp2Dym.PortID, channRollApp2Dym.ChannelID, rollapp2.Config().Denom)
	rollappIBCDenom := transfertypes.ParseDenomTrace(rollappTokenDenom).IBCDenom()
