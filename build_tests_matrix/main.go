...
func main() {
	githubActionMatrix, err := getGithubActionMatrixForTests(e2eTestDirectory, getTestToRun(), getExcludedTestFunctions())
	if err != nil {
...
func getGithubActionMatrixForTests(e2eRootDirectory, testName string, excludedItems []string) (GithubActionTestMatrix, error) {
	...
	err := filepath.Walk(e2eRootDirectory, func(path string, info fs.FileInfo, err error) error {
		...
		testCases, err = extractTestNames(f, excludedItems, testCases)
		if err != nil {
	...
	for _, testCaseName := range testCases {
		gh.Include = append(gh.Include, TestPair{
			Test: testCaseName,
		})
	}
...
func isTestFunction(f *ast.FuncDecl, suffix string) bool {
	return strings.HasPrefix(f.Name.Name, testNamePrefix) && strings.HasSuffix(f.Name.Name, suffix)
}
